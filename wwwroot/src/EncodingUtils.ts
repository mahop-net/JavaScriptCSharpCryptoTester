...
class EncodingUtils {
    ...
    // -------------------------------------------------------------------------------------------------
    static base64StringToArray(base64: string, nBlocksSize?: number): Uint8Array {
    ...
    // -------------------------------------------------------------------------------------------------
    static arrayToBase64String(aBytes: Uint8Array): string {
        let nMod3 = 2;
        let sB64Enc = "";
        ...
        for (let nIdx = 0; nIdx < nLen; nIdx++) {
            ...
            if (nMod3 === 2 || aBytes.length - nIdx === 1) {
                sB64Enc += String.fromCodePoint(
                    EncodingUtils.uint6ToB64((nUint24 >>> 18) & 63),
                    EncodingUtils.uint6ToB64((nUint24 >>> 12) & 63),
                    EncodingUtils.uint6ToB64((nUint24 >>> 6) & 63),
                    EncodingUtils.uint6ToB64(nUint24 & 63)
                );
                nUint24 = 0;
            }
        }
    ...
    // -------------------------------------------------------------------------------------------------
    static utf8ArrayToString(aBytes: Uint8Array): string {
        let sView = "";
        let nPart;
        const nLen = aBytes.length;
        for (let nIdx = 0; nIdx < nLen; nIdx++) {
